...
public abstract class Searcher {
     ...
     * found".  */
    public ArrayList<Integer> getGoalPath() 
    {
        if (goalNode == null)
            System.out.println("Goal node not found");
        return printPath(goalNode);
    }   
    ...
    private ArrayList<Integer> printPath(SearchNode node) 
    {
        RobotGraphNode rgNode = (RobotGraphNode) node;
        if (node.parent != null)
        {
            path.add(rgNode.getSpokeNum());
